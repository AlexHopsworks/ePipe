...
public:
  ...
  static string aliveState(string id, FileProvenanceRow row, string mlId, string mlType) {
    ...
    dataVal.AddMember("user_name",        rapidjson::Value().SetString(row.mUserName.c_str(), dataAlloc), dataAlloc);
...
std::list<boost::tuple<string, boost::optional<FileProvenancePK>, boost::optional<FPXAttrBufferPK> > > FileProvenanceElasticDataReader::process_row(FileProvenanceRow row) {
  ...
  if(row.mOperation == FileProvenanceConstants::H_OP_XATTR_ADD 
    ...
    if(row.mOperation == FileProvenanceConstants::H_OP_CREATE) {
      ...
      if(!FileProvenanceConstants::isReadmeFile(row) 
        && FileProvenanceConstants::isMLModel(row)) {
        ...
        && FileProvenanceConstants::isMLTDataset(row)) {
        ...
        && FileProvenanceConstants::isMLFeature(row)) {
