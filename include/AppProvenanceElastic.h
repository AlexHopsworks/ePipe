...
struct AppMovingCountersImpl : public AppMovingCounters{

  AppMovingCountersImpl(int stepInSeconds, std::string prefix) : mStepSeconds
...
struct AppMovingCountersSet : public AppMovingCounters{
  AppMovingCountersSet(){
    mCounters = {AppMovingCountersImpl(60, "last_minute"), AppMovingCountersImpl
                 (3600, "last_hour"), AppMovingCountersImpl(-1, "all_time")};
  }
  void check() override {
...
private:
  std::vector<AppMovingCountersImpl> mCounters;
};
...
private:
  ...
  AppMovingCountersSet mCounters;

  virtual void process(std::vector<AppPBulk>* bulks);

  void stats(std::vector<AppPBulk>* bulks);
