...
typedef vector<boost::optional<XAttrPK> > FPXAttrBKeys;

class FileProvenanceXAttrBufferTable : public XAttrTable {

public:
  FileProvenanceXAttrBufferTable() : XAttrTable("hdfs_file_provenance_xattrs_buffer") {
  }

  void cleanBuffer(Ndb* connection, FPXAttrBKeys& pks) {
    start(connection);
    for (FPXAttrBKeys::iterator it = pks.begin(); it != pks.end(); ++it) {
      boost::optional<XAttrPK> pk = *it;
      if(pk) {
        AnyMap a;
        a[0] = pk.get().mInodeId;
